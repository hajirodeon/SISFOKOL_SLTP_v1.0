<?php
/* $Id: db_stats.php,v 1.27 2001/10/15 22:27:32 loic1 Exp $ */
...
if ($server > 0) {
    ...
    $dbs     = @mysql_list_dbs() or mysql_die('', 'mysql_list_dbs()', '', 'main.php?lang' . $lang . '&amp;server=' . $server);
...
<form action="db_stats.php">
    ...
    while (list($db_name, $db_prop) = each($dbs_array)) {
        ...
        echo '        </td>' . "\n";
        echo '        <td>&nbsp;<a href="index.php?lang=' . $lang . '&amp;server=' . $server . '&amp;db=' . urlencode($db_name) . '" target="_parent">' . htmlentities($db_name) . '</a>&nbsp;</td>' . "\n";
